...
These analytical formulars enable the caluclation of parmets for
unagpped alignment. However, these formulars do not hold for gapped
alignments. Empirical studies showed that simiar theory holds for gapped
alignments as well \citep{pearson1998empirical,smith1985statistical,
altschul199627}. 
%
For gapped alignments, the parmetes for the distribution are estimated
empirically from the comaprison of random sequences
...
\citep{pearson1988improved,pearson1995comparison} to detmine the
significance of mataches of a query to a database of sequences.
...
\ref{fast_pval} compared null distribution generated by aligning a
100,000 random reads (empirical approach) and the null distribution
computed using this method (analytic approach).
...
\caption[Fast computation of p-values]{
  ...
  the $k$ $k=1$ distributions correspond to the fragment set.
...
expected to be significantly larger (e.g. the human genome). Using a
larger genome
...
procedure to determine the optimal number of fragments on a read. For
example, in the context of local alignments, using a  ... has .
...
For example, if the method described in section~\ref{} is used, the mean
and variance of aligning fragment of different lengths need to be
estimated.
%
...
approximations.
%
Similar to BLAST statistics \citep{}, it could also provide an efficient
procedure for determining the p-value of aligning a SMURF-seq read.
...
%% refine step
Further, the candidate regions found in the search step could be further
...
A SMURF-seq reads could have fragments that do not or
originate from the reference genome (due to DNA molecules from
contaminants or regions of high-sequencing errors on a read),
% Too small reads for clt to hold
or a SMURF-seq read could have too few fragments for the normal
approximation to hold.
%
Developing a mapping tool for SMURF-seq reads would require taking in
to consideration these and several other factors, such as fragments
aligning to several locations due to repeats on the genome, that we have
not considered here.
